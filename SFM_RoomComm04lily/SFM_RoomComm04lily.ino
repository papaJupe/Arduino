...
/* SFM RoomComm04lily  -- Roo control via serial port from iOS/Andr
    ...
    v 0.4 adds sensor reading, display some data to text field, added
    [red BOB and lily variations for different LED pins]
    added 'const' to all img files (load into flash), ? helps flakiness
...
int led = 13;  // 13 on SimbLily, 2 on Spark BOB
...
void setup() {
  ...
  SimbleeForMobile.advertisementData = "v 0.4"; // total 15 ch allowed
...
void loop() {  // loop farms out all action to update(), ? speedier
  ...
  if (count == 2) // get sensor data once in a while
  {
    ...
    delay(50);  // probably could be smaller
    int i = 0;
    byte inB = 0;
...
//  #define downLeft 7
  ...
  //  can't add more images: compiles, but app fails to open; might
  // help to add const to unsigned int var in .h file
  ...
  // 3 control rects, work as buttons
  ...
  //// will rect still work if I draw transp image over it, andr, iOS Y if not on it
  ...
  // update call sets desired init values

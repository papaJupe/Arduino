...
/* SFM RoomCommTest02  -- experiments for R control from iOS/Andr
    ...
    to commands: completed control of power, drive, stop, brush, clean
  ...
  todo: for v. 0.4 -- sensor feedback display to second field
...
char data[120] = "";  // string to show sensor 'data' in 2nd frame
...
void setup() {
  ...
  SimbleeForMobile.advertisementData = "v 0.3"; // total 15 ch allowed
...
uint8_t textData;  // reports sensor feedback
uint8_t textStop; // label for center square

uint8_t dataRect;  // wht frame around textControl & textData fields
...
#define upRt 6  // only allowed 6-7 img, adding more crashes app
  ...
  // I've not set events for brush or clean, but they still work same
...
{ // both to the board hardware and display on the phone app
  ...
  // if only one event can affect var, or I don't care which, send the new value
  // SimbleeForMobile.updateValue(britefield, brite);

  // on any update I start data field with speed
  sprintf(cont, "     spd= %d ", speed);  // put int's as chars in array

  // stop any action if center pressed, slight delay needed
  ...
  // power ON -- press toggles action ON/OFF
...
void drive(int velo, int radi)
{
  ...
  char temp[28];  // add current values to cont display
  sprintf(temp, " spd %d %d, rad %d %d ", cmd[1], cmd[2], cmd[3], cmd[4]);
  strcat(cont, temp);
  Serial.write( cmd, 5 );
}  // end drive

// ? called by .process, need this to power off R onDisc, N

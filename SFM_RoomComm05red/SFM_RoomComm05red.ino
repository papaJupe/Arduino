...
/* SFM RoomComm05  -- Roo control from iOS/Andr
...
void loop() {  // loop farms out all action to update(), ? speedier
  ...
  if (count == 2)  // get sensor data once in a while
  {
    ...
    if (Serial.available()) {
      ...
      while (Serial.read() != -1)
        ;  // clear the input buffer, should be already
...
//uint8_t textData;  // report sensor data in, declared global
...
{  // both to the board hardware and display on the phone app
  ...
  // power OFF
  else if (eventId == rectPower && powerMode == 1) {  // if power up already, stop and power down
    memset(cont, 0, 62);
  ...
  else if (eventId == rectBrush && powerMode) {  // power on all motors
    char spin[] = " brush up";
    strcat(cont, spin);
    Serial.write(138);
    Serial.write(7);
...
void ui_event(event_t &event) {  // just resets vars according to event property; kept small
  // for speed --> farm out most actions to loop & update()
  // each event type has hidden unique byte id value I guess
  ...
  // event.types to do various stuff, would rather send var to update() or loop
...
String sensorsAsString() {  // to display more data , would need bigger data[]
  return
    ...
    //      " dist:" + distance() +
    //      " angl:" + angle() +
    (" chgState: " + String(sensIn[CHARGINGSTATE] & 0xff) + " volt: " + String((sensIn[VOLTAGE_HI] << 8) | (sensIn[VOLTAGE_LO] & 0xff)) + " curr: " + String(int16_t((sensIn[CURRENT_HI] << 8) | (sensIn[CURRENT_LO] & 0xff))) + " temp: " + String(sensIn[TEMPERATURE] & 0xff));
  //      " chrg:" + charge() +
  //      " capa:" + capacity() +

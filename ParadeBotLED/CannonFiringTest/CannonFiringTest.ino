/*
  cannon firing test -- air cannon uses 8 segments, mod of movement code of
  parade bot's 4 tracks
  static test writes 4 different colors to segments of 5 lites so we can count each
  segment exact number;
  trial pattern here for cannon segments to move depending
  on firing status;

  Uno test: simple static pattern works, ? one long string moving
*/
...
#define NUM_LEDS 298
#define DATA_PIN 7  // control pin going to LED string
#define FIRING_PIN 12   // input signal, high if firing
...
void setup()
{
  ...
  FastLED.setBrightness(50);  // 0-255, use min value needed for environment
...
void loop()
{
  ...
  // moving 10 lite segments (some color, some blk)
  // subtracting ct shifts forward
  for (int i = 0; i < NUM_LEDS; i++ )
  {
    if (digitalRead(FIRING_PIN))    // true when 'firing' pin pulls high
    ...
    } // end else make color segments

  }  //  end fill string with color
...
// cycle lites in N lite segments in fwd/reverse direction:
...
// # of loop counts must = segment length (mod used in formula)
// eg count > 9 --> count = 0 if using mod 10

/*
  ...
  initial test uses 4 different colors to 5 lite segments to clarify each
  segment exact number;
  todo: read some control pin from sidecar (firing T/F);
   write patterns for cannon segments to move depending
   on firing status;
...
void setup()
{
  ...
  digitalWrite(GOING_PIN, LOW);  // low=false, HIGH = true below
...
void loop()
{
  //   --  bools from labView/cRIO via input pins control
  //        segment movements
...
//  }
  ...
  if (digitalRead(IS_FIRING_PIN) == 1) // true when pin pulls high
  {
  ...
  for (int i = 24; i < 49; i++ )   // lite segment affected
   ...
   // end for moving segment 2

  //Segment 3 (Wave Bottom Left)
  for (int i = 49; i < 64; i++ )   // lite segment affected
  
    {
      led2[i] = green;
    }
   // end for moving segment 3
  ...
  for (int i = 100; i < 117; i++ )   // lite segment affected
   ...
   // end for moving segment 5

  //Segment 6 (Mini Barrel Right)
  for (int i = 118; i < 137; i++ )   // lite segment affected
  ...
  for (int i = 214; i < 246; i++ )   // lite segment affected
  {
   ...
   // end for moving segment 9

  //Segment 10 (Wave Bottom Right)
  for (int i = 246; i < 258; i++ )   // lite segment affected
  {
    led2[i] = yellow;
  } // end for moving segments 10
}  // end if firing
else
{

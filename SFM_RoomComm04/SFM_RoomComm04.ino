...
/* SFM RoomComm04  -- working R control from iOS/Andr
    ...
    v 0.4 adds sensor reading, display some data to text field
...
int led1 = 2;  // 13 on SimbLily, 2 on Spark BOB
...
void setup() {
  ...
  pinMode(led1, OUTPUT);  // flash indicates serial request sent
...
void loop() {  // loop farms out all action to update(), ? speedier
  ...
  count++;
  if (count == 10) digitalWrite(led1, LOW);

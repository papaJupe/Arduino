/*
  ...
  OR attach the center pin of pot to PinIn, and the outside pins to +5V and ground,
   to display V range
  Uses: Liquid Crystal Nx2 display, map, analRead, expon moving average, round
  mapping here for unk model maxbotic found ChaThe drawer 2502
*/
...
void loop()
{
  ...
  newAvg = round((sensorV * 0.2) + (prevAvg * 0.8));  // update expon MA of 5 reads
  ...
  inch = map(newAvg, 59, 500, 11, 98); // mapping AR tick to inch for MaxBot MB1010
  // inch = map(newAvg, inlo:21, inhi:204, outl:7, outh:79); //ticks to inch, MaxBot MB1046/1122

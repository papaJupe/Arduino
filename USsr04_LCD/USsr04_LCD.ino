...
#define IN 0      //Inch,        get displayed otherwise, var not used
...
int NUM_KEYS = 5;
int adc_key_in;   // need loop code to read keys or make fx()
int key = -1;
int oldkey = -1;
...
{  static uint16_t distAvg = 69;
  ...
  lcd.clear();
  lcd.setCursor(0, 0);  // need mod if printing inches
  lcd.print("distance : ");
  lcd.print(avgDist);
  if(CM==1)
  lcd.print(" CM");

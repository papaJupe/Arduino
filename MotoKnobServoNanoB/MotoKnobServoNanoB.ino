...
/* MotoKnobServoNanoB -- Mead's mod to control a motor speed or servo using map function
  ...
  changed R channel to one direction, CCW, so 0 (stop) is 1500 ms, full on (1024) is 544
...
void loop()
{
  ...
  valR = map(newAvgR, 5, 1020, 1500, 544); // scale pot val to uS for servo (off to full reverse)
  ...
  valL = map(newAvgL, 5, 1020, 180, 0); // scale it to degrees for servo (full range = 0 to 180 or partial range)
  // servo position center = 90, pulse = 1500 uS, motor still
  // 70-110 gives range of 1280 - 1680 uS

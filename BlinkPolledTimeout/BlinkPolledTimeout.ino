...
void setup() {
  ...
  // STEP4: wait for OFF timeout to assure the led is kept off for (n))
  oneShotMs timeoutOff(2000);
  while (!timeoutOff) { yield(); }

  // Done with STEPs, do other stuff
  halfPeriod.reset();  // halfPeriod is global, so it gets init on sketch start. 
  // Clear it here to make it ready for loop, where it's actually used.
}

...
//
// Changelog:
//     2011-07-31 - initial release; no changes needed for FRC board compatibility ?
...
/** Default constructor, uses default I2C address set in .h
 ...
 * @see ADXL345_DEFAULT_ADDRESS &_LOW & _HIGH settings in .h
 */
 ...
 */
void ADXL345::initialize() // starts calling I2Cdev functions -- just uses def. I2Cdev constr.
{
...
bool ADXL345::testConnection() 
 {
  ...
  Serial.print("device ID is: "), Serial.println(deviceid, HEX);
  return getDeviceID() == 0xE5; // (229) for all 345 boards
  }
 ...
 * @return Device ID (should be 0xE5, 229 dec, 345 oct <== cute!)
 * @see ADXL345_RA_DEVID -- register 0x00 holds it
 */
...
/** Get/Set measurement data rate.
 * 0x7 =  12.5Hz
 * 0x8 =  25Hz, scaling by factors of 2, so:
 * 0x9 =  50Hz  (9 returned means 50 hz rate)
 * 0xA = 100Hz (10 returned by getRate means 100hz meas rate)
 * @param rate New data rate (0x0 - 0xF)
 * @see ADXL345_RATE_100  --> 100 means 100 hz
 * @see ADXL345_RA_BW_RATE
 * @see ADXL345_BW_RATE_BIT
 * @see ADXL345_BW_RATE_LENGTH
 */
...
/** Get X-axis acceleration measurement.
...
/** Get Y-axis acceleration measurement.
...
/** Get Z-axis acceleration measurement.

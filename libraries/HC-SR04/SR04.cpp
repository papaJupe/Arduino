...
SR04::SR04(int echoPin, int triggerPin) {
    ...
    // this fx called by examples sketch
long SR04::Distance() {
    long d = 0;
    _duration = 0;
...
long SR04::DistanceAvg(int wait, int count) {
  ...
  // augment count to get some extra values; will subtract extremes later
    ...
    // subtract highest and lowest values
    avg -= (max + min);
    // calculate average using the middle, hopefully more avg values
    avg /= count;
    return avg;
}

...
//accepts a 16 bit value and maps it down to the timer for maximum resolution
extern void   pwmWriteHR(uint8_t pin, uint16_t val);
extern bool   SetPinFrequency(int8_t pin, uint32_t frequency);
//does not set timers responsible for time keeping functions
extern bool   SetPinFrequencySafe(int8_t pin, uint32_t frequency);
//gets the PWM resolution of a pin in base 2, 0 is returned if the pin is not connected to a timer

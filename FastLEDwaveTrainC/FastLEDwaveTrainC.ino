...
byte pinOuts[] = {9, 10, 11}; //  these are [1's,2's,4's] bin output pins to LEDs
     //  could use for 1s,2s,4s,8s; but my 3 bit switch only goes 0-7
...
{   // count enables pulsing: flashes (count max)-1 trains, then dark for pulse*period
  ...
  // optional: binary display mode on LEDs, low endian I/O
  ...
  switch (mode)  // byte can be used when int expected
  {
    ...
    case 5: // mode 3 w/ longer pulsing
      {
        liteOn = 5;
        liteOff = 20;
        leds[0] = 0x99ffff; // sl bluish

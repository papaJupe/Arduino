/*
 WiFiEsp example: WebServerLed -- runs on Ardu with ESP shield, same target as 
 all wifiESP examples
...
void setup()
{
  ...
  WiFi.init(&Serial2);    // initialize ESP module comm on Ser2
...
void loop()
{
  ...
  if (client) {                           // if you get a client,
    Serial.println("New client");         // print a message to serial port
    ...
    while (client.connected()) {          // loop while the client's connected
      if (client.available()) {       // if there's byte to read from the client,
        ...
        // if you got two newline characters in a row
        // that's the end of the HTTP request, so send a response
        if (buf.endsWith("\r\n\r\n")) {
          sendHttpResponse(client);
          // Serial.println();
          break;
             }  // end if end of request

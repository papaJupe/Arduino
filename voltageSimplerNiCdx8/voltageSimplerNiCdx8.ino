...
const float targVolt= 9800; // discharge target in mV, varies with batt type, depth you want
                            // for NiCd 8 pack this is 50% dc at C/5 -- 1.225 v each cell
float voltRegul = 7055;   // possible (?int) value of regulated voltage offset, 10500-voltRegul <= 5K
...
float mult = 4.840;      // multiplier empirically determined from measured v across VR, may vary betw boards
...
{  
  ...
  if (Serial.available())
   { 
     ...
     else      // any char but y ends the setup; need braces? not for one liner
  ...
  if (done == 1)  // main loop does this block repeatedly when done
   {
      ...
      for (byte i=0; i<=6; i++)  // play finish tone every 15 sec
        {
         tone(11, 2700);   // don't need pin config for toner
         delay(500);
         noTone(11);
         delay(200);
        } // end tone loop
